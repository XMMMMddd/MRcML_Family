  ...
  alpha_init = 0.0, max_iter = 100, tol = 1e-6
    ...
    for (k in 1:max_iter) {
        ...
        for (j in 1:J) {
            ...
            term1_inv <- inv_beta_sigma_exp_j +
        ...
        for (j in 1:J) {
            ...
            term_middle <- inv_beta_sigma_out_j
...
mr_family <- function(
  ...
  alpha_init = 0.0, max_iter = 100, tol = 1e-6
    ...
    p_value <- 2 * pnorm(-abs(a[[1]] / b$std_error))
    results <- tibble(alpha_point = a[[1]], alpha_se = b$std_error, p_value = p_value)
    return(results)
}
